...
it.each(['no-match', 'failed', 'unavailable'])('offers editable image search after refresh %s without saving on open or close', async kind => {
  const h = harness(); expect(h.find('Search exercise images')).toBeUndefined();
