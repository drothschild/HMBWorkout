...
describe('Resume during resting: relaunch reconciliation', () => {
  // Boot dispatches Resume unconditionally after rehydrating (_layout.tsx). A
  // process kill mid-rest leaves phase resting with a live or expired deadline
  // and no armed alert in the new process, so Resume is where the engine must
  // reconcile: re-arm a live rest, recover the phase from an expired one.
