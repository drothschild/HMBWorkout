...
describe('engine: dispatch loop with effect executors', () => {
  ...
  describe('state retention across phases: invalid events preserve state (M4)', () => {
    const phaseTests = [
      ...
      { phase: 'warmup' as const, invalidEvent: () => ({ tag: 'SkipRest' as const }), description: 'warmup' },
      { phase: 'working' as const, invalidEvent: () => ({ tag: 'SkipRest' as const }), description: 'working' },
