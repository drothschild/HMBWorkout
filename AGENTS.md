...
- `src/theme/` — design tokens: `ActionButtonColor` and `StatusColor` (solid
  fills that carry white text — buttons and the session error banner
  respectively — each darkened just enough to clear WCAG AA 4.5:1 against
  white) and the pure `contrastRatio` check that verifies them
