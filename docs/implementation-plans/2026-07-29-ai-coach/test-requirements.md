...
| **ai-coach.AC2.3** *(prompt half)* | integration | `src/ai/contextBuilder.test.ts` | `buildSystem(db, { kind: 'edit', routineId: 'routine-1' })` contains the routine name, the literal id `routine-1`, and the instruction to return drafts as revisions carrying `routineId: 'routine-1'`; an unknown `routineId` falls back to create-mode content without throwing. |
...
| **ai-coach.AC3.2** *(persistence half)* | integration | `src/ai/acceptDraft.test.ts` | A draft without `routineId` creates the routine, one exercise per distinct slugified title with the draft's `kind`, and `routine_exercises` rows whose `order` matches array index and whose target fields match the draft; the returned id matches `/^routine-\d+$/` (the `routine-${Date.now()}` id convention — not `crypto.randomUUID()`, which is not guaranteed on Hermes). |
| **ai-coach.AC3.2** *(store half)* | unit | `src/state/aiChatStore.test.ts` | With a pending draft, `acceptDraft()` calls the injected accept fn exactly once as `accept(db, pendingDraft)`, returns the routine id, and clears `pendingDraft`. |
| **ai-coach.AC3.3** | integration | `src/ai/acceptDraft.test.ts` | Accepting a draft carrying `routineId: 'routine-1'` against a seeded routine leaves exactly one routine with that id, updates its `name`, replaces its `routine_exercises` with the draft's entries, and returns `'routine-1'`. |
