...
Approved interaction: after refresh has no match, open an in-app search screen with editable terms, image thumbnails, and explicit selection. Preserve the current exercise image until a chosen replacement saves successfully.
...
3. Offer Search images after no-match or refresh failure. Save a selected URL through the existing validated download/override transaction under the photo-operation lock. Preserve the current image on cancellation or failure.
